...
|2022_TSP [paper](https://ieeexplore.ieee.org/document/9713943)  |Federated matrix factorization:<br>algorithm design and application to data clustering.  |[code](https://github.com/metastableB/kfed/)  | $F(W, \{H_p\}) = \sum_{p=1}^P \omega_p F_p(W, H_p) $<br>$\text{s.t.} \quad W \in \mathcal{W}, \ H_p \in \mathcal{H}_p, \ \forall p \in \mathcal{P}$<br>$\Phi_p(X_p, WH_p)/\omega_p + R_H(H_p)/\omega_p + R_W(W)$ | Content Cell  |
